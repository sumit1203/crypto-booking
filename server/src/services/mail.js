...
const sendBookingInfo = async (booking, { from, to }) => {
  try {
    const { personalInfo, roomType, status, fromDate, toDate, remaindingMinutes } = booking;

    const html = informationBody({ personalInfo, from: fromDate, to: toDate, roomType, status, remaindingMinutes });
